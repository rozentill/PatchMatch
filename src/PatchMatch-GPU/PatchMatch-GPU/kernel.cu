...
__host__ void patchmatch_program() {

	/*** Definition and initialization ***/

	/* cuda time count method*/
	cudaEvent_t start, stop;
	float elapsedTime;

	/* cpu time count method*/
	...
	ofstream fout(image_path + "log-gpu.txt");
	...
	cout << "Finished finding ann. Time : " << endl;
	...
	c = reconstruct_center(a, b, newann, patch_w);
	Mat errormap = reconstruct_error(a, b, newann, patch_w);
	Mat flowmap = reconstruct_flow(a, b, newann, patch_w);
	Mat energymap = reconstruct_energy(a, b, newann, newannd, patch_w);
	...
	Mat grayerrormap;
	cvtColor(errormap, grayerrormap, CV_BGR2GRAY);

	namedWindow("errormap", CV_WINDOW_AUTOSIZE);
	imshow("errormap", grayerrormap);
	namedWindow("energymap", CV_WINDOW_AUTOSIZE);
	imshow("energymap", energymap);
	namedWindow("flowmap", CV_WINDOW_AUTOSIZE);
	...
	imwrite(image_path + "bike_flow_gpu.png", flowmap);
