...
__global__ void patchmatch(int * a, int * b, unsigned int *ann, int *annd, int * params) {
	...
	int aew = a_cols - patch_w + 1;
	int aeh = a_rows - patch_w + 1;
	...
	if (ax < a_cols&&ay < a_rows) {
		...
		for (int iter = 0; iter < pm_iters; iter++) {
			...
			for (int jump = 8; jump > 0; jump /= 2) {
				...
				if ((ax - jump) >= 0)//left
				{
					unsigned int vp = ann[ay*aew + ax - jump];//the pixel coordinates in image b
				...
				if ((ay + jump) < aeh)//up
				{
					unsigned int vp = ann[(ay + jump)*aew + ax];//the pixel coordinates in image b
...
int main()
{
	...
	string test_a = "bike_a.png";
	string test_b = "bike_b.png";
	string image_path = "D:/MSRA/Code/Project/Artistic-Filter/Artistic Filter/Artistic Filter/files/images/";


	Mat a, b, c;
	int * newannd, * a_host, *b_host, *a_device, *b_device, *annd_host, *annd_device, *params_host, *params_device;
	...
	a = imread(image_path + test_a);
	b = imread(image_path + test_b);
	...
	dim3 blocksPerGrid(aew / 32 + 1, aeh / 32 + 1, 1);
	...
	annd_host = (int *)malloc(sizeOfAnn * sizeof(int));
	newann = (unsigned int *)malloc(sizeOfAnn * sizeof(unsigned int));
	newannd = (int *)malloc(sizeOfAnn * sizeof(int));
	...
	cudaMalloc(&annd_device, sizeOfAnn * sizeof(int));
	...
	cudaMemcpy(annd_device, annd_host, sizeOfAnn * sizeof(int), cudaMemcpyHostToDevice);
	...
	cudaMemcpy(newannd, annd_device, sizeOfAnn * sizeof(int), cudaMemcpyDeviceToHost);
	...
	c = reconstruct(a, b, newann, patch_w);
