...
void patchmatch(Mat a, Mat b, unsigned int **&ann, int **&annd) {
	...
	for (int ay = 0; ay < aeh; ay++) {
		for (int ax = 0; ax < aew; ax++) {
			...
			annd[ay][ax] = dist(a_pixel, b_pixel, ax, ay, bx, by);
...
/* average voting */
Mat reconstruct_average(Mat a, Mat b, unsigned int ** ann) {
	Mat a_recon;
	a.copyTo(a_recon);
...
int main()
{
	...
	imwrite(image_path+"bike_cpu_corner_nearest.png", a_recon);
